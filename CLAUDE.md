...
**Track B（执行 / 实时轨道，未实施）**：官方模块化 SDK `binance-sdk-derivatives-trading-usds-futures` + `binance-sdk-spot` 是实时 WS 行流 + 下单 + user-data stream 的预设基础（`dataflows/binance.py` docstring 已留 "Track B (execution) will bring the official SDK"）。**当前不迁 SDK**——对分析层 REST 公共行情无净收益，且 SDK 未文档化支持 `socks5h://`（需额外 `requests[socks]`/`aiohttp-socks`）。执行轨道单独启动时再引入。

...
suite (delegated to :mod:`tradingagents.backtest.metrics`).
...
* **Honest about LLM cost/non-determinism.** A :class:`~tradingagents.backtest.cache.DecisionCache`
...
from tradingagents.backtest.cache import CachedDecision, DecisionCache
from tradingagents.backtest.metrics import BacktestMetrics, compute_metrics
...
class _GraphLike(Protocol):
    """Structural type the engine needs from a TradingAgentsGraph."""
...
def run_backtest(
    ...
    graph:
        ...
        returning ``(final_state, rating)`` (a :class:`TradingAgentsGraph`).
    ticker:

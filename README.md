...
> 面向研究的**多智能体 LLM 量化交易框架**——设计方法学源自 100+ 篇 LLM 金融推理 / 多智能体 / 量化风控 / 对抗安全文献（见[研究基础](#研究基础与文献支撑)，完整文献见 [REFERENCES.md](REFERENCES.md)）。
>
> 包名 / import / CLI 命令统一为 `yiagents`，环境变量前缀为 `YIAGENTS_*`，数据目录为 `~/.yiagents/`。

---

## 这是什么

YiAgents 用一组分工明确的 **LLM 智能体**模拟真实交易公司的运作：基本面 / 情绪 / 新闻 / 技术分析师产出观点，多空研究员结构化辩论，交易员给出提案，风控团队与组合经理做最终裁决。在此之上，框架叠加了一层**确定性量化风控**（Kelly 仓位 / ATR 止损 / 熔断 / CVaR）和一套**四档验证 + 回测闸门**流程，把"研究玩具"往"可上线工程"推。
...
> 框架内置多 LLM 提供商、LangGraph 编排、多语言、记忆闭环、检查点恢复等基础能力。
...
YiAgents 的设计方法学基于上表所列文献。若本框架对你的工作有帮助，请引用：

...
>
> **展示名为 YiAgents**；为兼容上游与既有配置，代码内部的 **Python 包名仍为 `tradingagents`**、**CLI 命令仍为 `tradingagents`**、**环境变量前缀仍为 `TRADINGAGENTS_*`**。导入、命令、`.env` 均无需改动。
...
| **量化风控叠加层** | [tradingagents/risk/](tradingagents/risk/) | `kelly.py` / `atr_stop.py` / `breaker.py` / `cvar.py` / `manager.py`，LLM 定方向、数学定仓位与止损 |
| **回测引擎与闸门** | [tradingagents/backtest/](tradingagents/backtest/) | `engine.py` / `metrics.py`（含 Deflated Sharpe）/ `validation_gate.py`（PASS/FAIL 判定）/ `ic.py`（信息系数） |
| **监控仪表盘** | [tradingagents/monitoring/dashboard.py](tradingagents/monitoring/dashboard.py) | HTML 仪表盘，浏览器直开 |
| **浏览器券商执行层** | [tradingagents/execution/browser_broker.py](tradingagents/execution/browser_broker.py) | 桩实现 + 全局 kill switch（`TRADINGAGENTS_KILL_SWITCH`） |
| **多供应商数据路由** | [tradingagents/dataflows/](tradingagents/dataflows/) | yfinance / Alpha Vantage / FRED / Polymarket / Reddit / StockTwits / 浏览器另类数据 |
| **FinCoT 结构化提示词** | [tradingagents/default_config.py](tradingagents/default_config.py) | `fin_cot_prompts`：去人格化的"任务→推理步骤→输出约束"提示，默认关 |
...
数据层 (yfinance/AV/FRED/Polymarket)
   ...
   [可选] 量化风控叠加（risk_enabled）→ 仓位/止损/敞口再校准
        │
   决策 + 记忆闭环（写入 ~/.tradingagents/memory/）
```
...
```bash
export TRADINGAGENTS_LLM_PROVIDER=deepseek       # openai/google/anthropic/deepseek/groq/ollama/openai_compatible
export TRADINGAGENTS_DEEP_THINK_LLM=deepseek-chat
export TRADINGAGENTS_QUICK_THINK_LLM=deepseek-chat
export TRADINGAGENTS_OUTPUT_LANGUAGE=Chinese     # 分析师报告与最终决策输出语言
```

> 配置项与类型强制的 env 映射见 [tradingagents/default_config.py](tradingagents/default_config.py)（`_ENV_OVERRIDES`）。拼错的布尔/数值会在启动时报错，而非静默用默认值。
...
# DEFAULT_CONFIG 已自动套用 TRADINGAGENTS_* 环境变量覆盖
...
全部配置项见 [tradingagents/default_config.py](tradingagents/default_config.py)。
...
**决策日志（默认开启）：** 每次完成的运行把决策追加到 `~/.tradingagents/memory/trading_memory.md`。下次同 ticker 运行时，自动拉取实现收益（含相对基准的 alpha）、生成反思，并把最近同 ticker 决策与跨 ticker 教训注入组合经理提示，形成"吃一堑长一智"闭环。路径用 `TRADINGAGENTS_MEMORY_LOG_PATH` 覆盖。

**检查点恢复（opt-in）：** 用 `--checkpoint` 开启，LangGraph 在每个节点后存档，崩溃/中断可从最后一个成功步骤续跑，成功完成后自动清理。按 ticker 的 SQLite 库位于 `~/.tradingagents/cache/checkpoints/<TICKER>.db`（`TRADINGAGENTS_CACHE_DIR` 覆盖）。

```bash
tradingagents analyze --checkpoint           # 本轮启用
...
```text
.
├── tradingagents/            # 核心包（内部包名，对外 import 名）
...
降低波动的手段：调低 `temperature`（`TRADINGAGENTS_TEMPERATURE`），或显式选非推理模型（Custom model ID）。已确定性化的部分：分析公司身份在 agent 运行前由 ticker 解析锁定；市场分析师的精确价格/指标取自已校验的数据快照。

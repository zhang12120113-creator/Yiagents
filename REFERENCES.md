...
| 机构持仓 / 空头兴趣（13F，B2.1 预留） | Asquith, Pathak & Ritter (2005) "Short Interest, Institutional Ownership, and Stock Returns"（JFE） | B2.1 待实施（EFTS 按 CUSIP 检索 13F-HR 命中广度代理 / 商业源聚合） |

该 category 默认关=字节等价：未开 `YIAGENTS_SEC_OWNERSHIP` 时 fundamentals 分析师的工具列表 / prompt / 能力字节不变；开启后仅追加两支供其选用的信号工具 + 一段降级指引，不触任何 agent 的输入/能力/深度，不引入新随机性。
